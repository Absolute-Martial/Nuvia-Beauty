...
class ImportDemoData extends Command
{
    ...
    {
        $sourcePath = __DIR__ . '/../../stubs/sql/' . config('shop.dummy_data_path');
        $filesystem = new Filesystem();

        if (! $filesystem->exists($sourcePath)) {
            info('Legacy demo SQL fixtures are not bundled in the clean Nuvia Beauty repo.');
            info('Skipping demo import.');
            return self::SUCCESS;
        }

        info('Copying necessary files for seeding....');

        $filesystem->copyDirectory($sourcePath, public_path('sql'));
    ...
    public function seedDemoData()
    {
        ...
        if (config('shop.dummy_data_path') !== 'chawkbazar') {
