...
class ProductController extends CoreController
{
    ...
    public function importProducts(Request $request)
    {
        ...
        if (isset($shop_id)) {
            $products = $this->withStoredImportFile(
                $uploadedCsv,
                'products-' . $shop_id . '.' . $uploadedCsv->getClientOriginalExtension(),
                fn (string $tempPath) => $this->repository->csvToArray($tempPath)
            );
    ...
    public function importVariationOptions(Request $request)
    {
        ...
        if (isset($user->id)) {
            $attributes = $this->withStoredImportFile(
                $uploadedCsv,
                'variation-options-' . Str::random(5) . '.' . $uploadedCsv->getClientOriginalExtension(),
                fn (string $tempPath) => $this->repository->csvToArray($tempPath)
            );
