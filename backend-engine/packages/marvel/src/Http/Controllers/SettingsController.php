...
class SettingsController extends CoreController
{
    ...
    public function index(Request $request)
    {
        ...
        $maintenance = data_get($data, 'options.maintenance', []);
        $maintenanceStartRaw = data_get($maintenance, 'start');
        $maintenanceUntilRaw = data_get($maintenance, 'until');
        $formattedMaintenance = null;

        if (!empty($maintenanceStartRaw) && !empty($maintenanceUntilRaw)) {
            $formattedMaintenance = [
                "start" => Carbon::parse($maintenanceStartRaw)->format('F j, Y h:i A'),
                "until" => Carbon::parse($maintenanceUntilRaw)->format('F j, Y h:i A'),
            ];
        }
