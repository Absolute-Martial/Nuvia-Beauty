...
const SideBarMenu = () => {
  ...
  const { sidebarLinks } = vendorSiteSettings;
  ...
  return (
    <>
      {!isEmpty(sidebarLinks?.vendorDashboard) ? (
        <div className="flex flex-col px-5 pt-10 pb-3">
          ...
          <div className="space-y-2">
            {sidebarLinks?.vendorDashboard?.map((item, index) => {
              if (!hasAccess(item?.permissions, permissions)) return null;
              return (
                <Link
                  href={item?.href}
                  key={index}
