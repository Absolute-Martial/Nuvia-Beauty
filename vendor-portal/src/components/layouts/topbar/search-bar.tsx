...
import { vendorSiteSettings } from '@/settings/vendor.site.settings';
import {
  getAuthCredentials,
  hasAccess,
  platformOnly,
...
const SearchBar: React.FC<IProps> = ({}: IProps) => {
  ...
  const handleSearch = (text: string) => {
    ...
    const {
      sidebarLinks: { vendor, shop: shopSideLink, vendorDashboard },
    } = vendorSiteSettings;

    const vendorLinks = extractHrefObjects(Object.values(vendor));
    const shopLinks = extractHrefObjects(Object.values(shopSideLink));
    const vendorDashboardLinks = extractHrefObjects(
      Object.values(vendorDashboard)
    );
    let searchAbleLinks = [];
    let flattenShop = [];
    ...
    switch (true) {
      ...
      case isEmpty(shop) && hasAccess(platformOnly, currentUserPermissions): // This execute when user is and platform admin but not under a shop route
        searchAbleLinks = vendorLinks;
        break;

      case isEmpty(shop) && hasAccess(vendorOwnerOnly, currentUserPermissions): // This execute when user is and vendor but not under a shop route
        flattenShop = [...vendorDashboardLinks];
        me?.shops.map((s) =>
      ...
      default:
        searchAbleLinks = getAuthorizedURL(vendorLinks);
        break;
    }
